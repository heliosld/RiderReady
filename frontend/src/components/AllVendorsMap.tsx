...
export default function AllVendorsMap({ locations, height = '600px' }: AllVendorsMapProps) {
  ...
  useEffect(() => {
    ...
    import('leaflet').then(async (L) => {
      ...
      const geocodeAddress = async (location: VendorLocationData, index: number): Promise<VendorLocationData & { latitude: number; longitude: number } | null> => {
      ...
      const validLocations = geocodedLocations.filter(loc => loc !== null) as (VendorLocationData & { latitude: number; longitude: number })[];
